...
namespace ChessChallenge.Application
{
    public class ChallengeController
    {
        ...
        Move GetBotMove()
        {
            try
            {
                Timer timer = new(PlayerToMove.TimeRemainingMs, PlayerNotOnMove.TimeRemainingMs, GameDurationMilliseconds, IncrementMilliseconds);
                Move move = PlayerToMove.Bot.Think(board, timer);
                return move;
            }
            catch (Exception e)
            {
            ...
            if (isPlaying)
            {
                ...
                if (PlayerWhite.IsBot && PlayerBlack.IsBot)
                {
                    ...
                    else if (autoStartNextBotMatch)
                    {
                        ...
                        boardUI.StopVisualizingBitboard();
                    }
                } else {
                    boardUI.StopVisualizingBitboard();
                }
